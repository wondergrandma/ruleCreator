...
def createEmptyDir(token, repo_id):
    ...
    for rules in config_user.DIR_NAMES:
        requests.post("http://10.50.64.5/api/v4/projects/"+f'{repo_id}'+"/repository/files/"+f'{rules}'+"_RULES%2Finicialize.txt?ref=main",
...
def create(data):
    ...
    if new_user:
        ...
        new_user = User(public_id=str(uuid.uuid4()), repository_id=new_repo, gitlab_id=gitlab_user_id, access_token=user_token, email=user_email, name=user_name, surname=user_surname, nick=user_nick, password=hashed_password.decode('utf-8'), mongo_db_id=mong)
