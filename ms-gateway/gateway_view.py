...
class SortRules(Resource):
    ...
    def post(current_user, self):
        ...
        create_str_json = json.dumps(current_user, cls=AlchemyEncoder)
        load_json = json.loads(create_str_json)

        response_json = json.loads(data)
        response_json["public_id"] = load_json
        update_json = json.dumps(response_json)

        req = requests.post(ms_rule_url + config_gateway.MS_RULES_API_ENDPOINT_DAT, update_json)
        return req.json()
...
class GetMongoData(Resource):
    def post(self):
        data = request.data
        req = requests.post(ms_rule_url + config_gateway.MS_RULES_API_ENDPOINT_MON, data)
        return req.json()

class DeletFileFromRepo(Resource):
    def post(self):
        data = request.data
        req = requests.post(ms_rule_url + config_gateway.MS_RULE_API_ENDPOINT_DEL, data)
        return req.json()
#---------------- MAIN RESOURCES ------------------
